...
void loadGameData(GameContext* game) {
    ...
    if (f) {
        ...
        if (fread(&tempScore, sizeof(int), 1, f) == 1) {
            game->highScore = tempScore;
        }
        if (fread(&tempLines, sizeof(int), 1, f) == 1) {
            game->bestLines = tempLines;
        }

        if (fread(&tempMusic, sizeof(int), 1, f) == 1) {
            game->menuMusicTrack = tempMusic;
        }
        if (fread(&tempRes, sizeof(int), 1, f) == 1) {
            game->menuResolution = tempRes;
        }
        ...
        if (fread(&tempSpeed, sizeof(int), 1, f) == 1) {
            game->menuAutoSpeed = tempSpeed;
        }
        if (fread(&tempLevel, sizeof(int), 1, f) == 1) {
            game->menuStartLevel = tempLevel;
        }
...
void checkHighScore(GameContext* game) {
    ...
    if (game->linesCleared > game->bestLines) {
        game->bestLines = game->linesCleared;
        changed = 1;
    }
}
...
void resetGameLogic(GameContext* game) {
    ...
    if (game->fallInterval < 50) {
        game->fallInterval = 50;
    }
...
static void rotatePiece(GameContext* game, int dir) {
    ...
    for (int i = 0; i < 6; i++) {
        if (game->currentPiece.type != 0 && abs(kicks[i][0]) == 2) {
            continue;
        }
...
static void holdPiece(GameContext* game) {
    if (game->state != STATE_PLAYING || game->isPaused || !game->canHold) {
        return;
    }
...
void handleInput(GameContext* game, SDL_Keycode key) {
    ...
    if (key == SDLK_m) {
        if (audioSystem.isMuted) {
            audioSystem.volume = audioSystem.previousVolume;
            audioSystem.isMuted = false;
        } else {
            audioSystem.previousVolume = audioSystem.volume;
        ...
        if (audioSystem.volume > 1.0f) audioSystem.volume = 1.0f;
        ApplyVolume();
    } else if (key == SDLK_f) {
        if (audioSystem.isMuted) audioSystem.isMuted = false;
        audioSystem.volume -= 0.1f;
        if (audioSystem.volume < 0.0f) audioSystem.volume = 0.0f;
        ApplyVolume();
    }
    ...
    if (game->state == STATE_MENU) {
        ...
        switch (key) {
            ...
            case SDLK_RETURN:
                if (game->gameInProgress && game->menuSelectedOption == 0) {
                    game->state = STATE_PLAYING;
                    game->isPaused = 0;
                } else if (game->menuSelectedOption == 0 + offset) {
                    resetGameLogic(game);
                } else if (game->menuSelectedOption == 1 + offset) {
                    game->state = STATE_SETTINGS;
                    game->menuSelectedOption = 0;
                } else if (game->menuSelectedOption == 2 + offset) {
