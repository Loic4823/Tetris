...
void loadBlockTextures(AppContext* app, int style) {
    char path[128];
    const char* styleFolder = (style == 0) ? "default" : "alt";
...
int initSDL(AppContext* app) {
    ...
    // --- CHARGEMENT DES 10 MUSIQUES ---
    // 1. tetris.mp3 (sans chiffre)
    app->musics[0] = Mix_LoadMUS("assets/music/tetris.mp3");
    
    // 2. tetris2.mp3 à tetris10.mp3
    char zikPath[64];
    for (int i = 1; i < 10; i++) {
...
void playMusicTrack(AppContext* app, int track) {
    Mix_HaltMusic();
    // Si l'index est valide (0 à 9), on joue
    if (track >= 0 && track < 10 && app->musics[track]) {
        Mix_PlayMusic(app->musics[track], -1);
    }
    // Si track == 10 (Aucune), ça reste coupé
}
...
void applyAudioVolume(GameContext* game) {
    if (game->isMuted) {
        Mix_VolumeMusic(0);
        Mix_Volume(-1, 0); // Coupe aussi les bruitages
    } else {
        Mix_VolumeMusic(game->masterVolume);
        Mix_Volume(-1, game->masterVolume); // Bruitages au même volume
    }
}
...
void renderSettingsMenu(AppContext* app, GameContext* game) {
    ...
    // 0. TEXTURE
    sprintf(buffer, "Texture: < %s >", (game->menuTextureStyle == 0) ? "Default" : "Alt");
    ...
    // 4. MUSIQUE (AVEC NOMS PERSONNALISÉS)
    const char* musicNames[] = {
        "Electro", "Orchestre", "Dramatique", "Techno", "Orgue Vent",
        "Metal", "Bass Techno", "Acapella", "Piano", "Japonais", "Aucune"
    };
    int trackIndex = game->menuMusicTrack;
    if (trackIndex < 0 || trackIndex > 10) trackIndex = 0; // Sécurité
...
void renderMenu(AppContext* app, GameContext* game) {
    ...
    if (app->menuBackground) {
        ...
        renderPiecePreview(app, 0, 50, 150); renderPiecePreview(app, 1, 650, 150);
        renderPiecePreview(app, 3, 620, 300); renderPiecePreview(app, 2, 80, 300);
