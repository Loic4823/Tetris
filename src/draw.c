...
void loadBlockTextures(AppContext* app, int style) {
    ...
    for (int i = 0; i < 7; i++) {
        if (app->blockTextures[i]) {
            SDL_DestroyTexture(app->blockTextures[i]);
        }
...
void cleanupSDL(AppContext* app) {
    ...
    if (app->ghostTexture) {
        SDL_DestroyTexture(app->ghostTexture);
    }

    if (app->menuBackground) {
        SDL_DestroyTexture(app->menuBackground);
    }
    if (app->settingsBackground) {
        SDL_DestroyTexture(app->settingsBackground);
    }
    ...
    for (int i = 0; i < 10; i++) {
        if (app->musics[i]) {
            Mix_FreeMusic(app->musics[i]);
        }
    }
    if (app->soundClear) {
        Mix_FreeChunk(app->soundClear);
    }
...
static void renderBlock(AppContext* app, int x, int y, int type, int isGhost) {
    ...
    if (isGhost) {
        if (app->ghostTexture) {
            SDL_RenderCopy(app->renderer, app->ghostTexture, NULL, &rect);
        } else {
            SDL_SetRenderDrawColor(app->renderer, 200, 200, 200, 100);
            SDL_RenderDrawRect(app->renderer, &rect);
        }
    } else {
        if (type >= 0 && type < 7 && app->blockTextures[type]) {
            SDL_RenderCopy(app->renderer, app->blockTextures[type], NULL, &rect);
        } else {
...
static void renderGhost(AppContext* app, GameContext* game) {
    ...
    while (!collision) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (bx < 0 || bx >= BOARD_WIDTH || by >= BOARD_HEIGHT || (by >= 0 && game->board[by][bx] != -1)) {
                collision = 1;
            }
        }
    }
...
void renderGame(AppContext* app, GameContext* game) {
    ...
    for (int x = 0; x <= BOARD_WIDTH; x++) {
        SDL_RenderDrawLine(app->renderer, BOARD_X_OFFSET + x * BLOCK_SIZE, BOARD_Y_OFFSET, BOARD_X_OFFSET + x * BLOCK_SIZE, BOARD_Y_OFFSET + BOARD_HEIGHT * BLOCK_SIZE);
    }
    for (int y = 0; y <= BOARD_HEIGHT; y++) {
        SDL_RenderDrawLine(app->renderer, BOARD_X_OFFSET, BOARD_Y_OFFSET + y * BLOCK_SIZE, BOARD_X_OFFSET + BOARD_WIDTH * BLOCK_SIZE, BOARD_Y_OFFSET + y * BLOCK_SIZE);
    }

    for (int y = 0; y < BOARD_HEIGHT; y++) {
        for (int x = 0; x < BOARD_WIDTH; x++) {
            if (game->board[y][x] != -1) {
                renderBlock(app, x, y, game->board[y][x], 0);
            }
        }
    }
    ...
    if (game->state == STATE_PLAYING) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (by >= 0) {
                renderBlock(app, bx, by, game->currentPiece.type, 0);
            }
        }
    }
...
void renderKeyConfigMenu(AppContext* app, GameContext* game) {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        if (game->keyConfigSelection == i && game->keyConfigColumn == 0) {
            sprintf(buffer1, "[ %s ]", rawName1);
        } else {
            sprintf(buffer1, "%s", rawName1);
        }

        if (key2 == 0) {
            if (game->keyConfigSelection == i && game->keyConfigColumn == 1) {
                sprintf(buffer2, "[ ... ]");
            } else {
                sprintf(buffer2, "...");
            }
        ...
        if (game->keyConfigSelection == i && game->keyConfigColumn == 0 && game->isRebinding) {
            c1 = waitingColor;
        }
        ...
        if (game->keyConfigSelection == i && game->keyConfigColumn == 1 && game->isRebinding) {
            c2 = waitingColor;
        }
...
void renderSettingsMenu(AppContext* app, GameContext* game) {
    ...
    if (game->menuTextureStyle == 1) {
        texName = "Alt";
    } else if (game->menuTextureStyle == 2) {
        texName = "MC";
    }
    ...
    if (audioSystem.isMuted || audioSystem.volume <= 0.01f) {
        sprintf(buffer, "Volume: < MUET >");
    } else {
        sprintf(buffer, "Volume: < %d%% >", (int)(audioSystem.volume * 100));
    }
    ...
    if (game->menuResolution == 1) {
        resText = "1200x1050";
    } else if (game->menuResolution == 2) {
        resText = "Plein Ecran";
    }
...
void renderMenu(AppContext* app, GameContext* game) {
    if (game->state == STATE_KEY_CONFIG) {
        renderKeyConfigMenu(app, game);
        return;
    }
    if (game->state == STATE_SETTINGS) {
        renderSettingsMenu(app, game);
        return;
    }
